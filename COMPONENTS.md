...
## AWWWARDS MOTION PASS (2026-06-01, 5th direction) — new + changed + promoted

| Component | Path | Purpose | Status |
|---|---|---|---|
| `HeroScroll` | `src/components/sections/HeroScroll.tsx` | THE centerpiece. Scroll-scrubbed "one capture, five modes" pinned film. Desktop+motion: GSAP ScrollTrigger pins the stage and scrubbing draws the capture marquee (DrawSVG), snaps the SPX handles, resolves OCR/HEX/DOM/SVG/SPX each into a blob-chip, then fans all five deck-style. Synced to Lenis (`lenis.on('scroll', ScrollTrigger.update)` + `lagSmoothing(0)`). Renders a STATIC SVG poster (scene drawn + 5 chips fanned + labelled) as first paint; GSAP dynamic-imported after mount. Gated by `gsap.matchMedia()` (desktop + `prefers-reduced-motion:no-preference`) and the `?motion=0` flag check: in reduce/motion-off NO ScrollTrigger is created (no pin/scrub/trap), the poster is the whole hero. `<=768` swaps to the interactive `ModeSwitcher`. Tall scroll reservation is CSS-gated (`.hero-spacer`/`.hero-stage`) for CLS 0. Replaces `Hero` as the home hero. | NEW (replaces `Hero` in page.tsx; `Hero.tsx` left orphaned/unused). |
| `MagneticButton` | `src/components/ui/MagneticButton.tsx` | Pill CTA that leans toward the cursor within a small radius and springs back (Framer `useMotionValue` + `useSpring`, critically damped stiffness 220 / damping 30, pull clamped to ~10px, transform-only). Visual styling mirrors `ui/Button` exactly (one pill system). Static / reduced / `?motion=0` / coarse pointer: magnet disabled, plain static pill, hover colours kept. Used on the hero CTAs. | NEW. |
| `KineticHeadline` | `src/components/motion/KineticHeadline.tsx` | One oversized headline. Per-word reveal on entry (GSAP SplitText) + scroll-velocity skew clamped to 6deg, eased back to 0 on settle, transform-only. Renders the plain `<h1>` text immediately (LCP-safe); GSAP + SplitText dynamic-imported after mount and SKIPPED entirely under `?motion=0` / reduced-motion (plain static headline). Hero headline only. `.kh-word` CSS masks each word. | NEW. |
| `Quirky` | `src/components/character/Quirky.tsx` | UPGRADED to spring physics: pupils + body lean now driven by Framer `useMotionValue` + `useSpring` (critically damped, NO overshoot) instead of per-frame React state (no re-render churn). Eyes track the cursor; the whole body leans toward it (rotate clamped 5deg + small translate) and settles; one-shot delight when a mode resolves. rAF-throttled pointer. Static / coarse-pointer behaviour unchanged (calm centred pose). API unchanged. | CHANGED (physics). |
| `Modes` / `Pricing` / `Footer` | `src/components/sections/{Modes,Pricing,Footer}.tsx` | The repeated below-the-fold items (5 mode rows, 2 pricing cards, footer grid) now reveal via native CSS scroll-timeline `.st-reveal` (compositor thread, zero JS) instead of the Framer `Reveal` wrapper on those specific items. Headers/steps/proof still use `Reveal`. | CHANGED. |
| `.st-reveal` CSS | `src/app/globals.css` + `ui-kit/components/motion/scroll-timeline-reveal.css` | Native `animation-timeline: view()` reveal (rise + sharpen). Triple-gated (`@supports` + reduced-motion + `html:not([data-motion=off])`); defaults visible. transform/opacity/filter only (CLS 0). | NEW, PROMOTED to ui-kit + INDEX row, same session. |
| `LenisProvider` | `src/components/providers/LenisProvider.tsx` | Now exposes the running instance on `window.__lenis` (only when smooth scroll is active) so `HeroScroll` can sync ScrollTrigger to Lenis. Cleared on teardown. | CHANGED. |
